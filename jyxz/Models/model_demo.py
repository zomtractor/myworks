...
# Version:          00
# Created:          2025/01/02
# Description:      使用MDTA加window attention 加 LEMF的模型
# ------------------------------------------------------------------
...
##modules
from Modules.DomnSample import Downsample
from Modules.UpSample import Upsample
from Modules.InputProj import InputProj
from Modules.OutProj import OutputProj
from Modules.Layers import MaskAttentionUformerLayer, BasicUformerLayer
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.output_proj_mask = OutputProj(in_channel=embed_dim, out_channel=1, kernel_size=3, stride=1)
        ...
        self.decoderlayer_3 = MaskAttentionUformerLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=depths[8],
        ...
        # mask decoder
        self.m_upsample_3 = upsample(embed_dim * 4, embed_dim)
        self.m_decoderlayer_3 = BasicUformerLayer(dim=embed_dim * 1,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=depths[8],
    ...
    def forward(self, x, mask=None):
        ...
        ##
        m_up3 = self.m_upsample_3(deconv2)
        m_deconv3 = self.m_decoderlayer_3(m_up3, mask=mask)
        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3, conv0], -1)
        deconv3 = self.decoderlayer_3(deconv3, m_deconv3, mask=mask)
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=32, depths=depths,
                                win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=False,
                                shift_flag=False)
