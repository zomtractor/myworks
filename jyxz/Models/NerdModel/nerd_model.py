...
from Modules.nerd_mlp import INR
...
if __name__ == "__main__":
    input_size = 512
    model_restoration = MultiscaleNet().cuda()
    input_ = torch.randn(1, 3, input_size, input_size).cuda()
    output_ = model_restoration(input_)
    print(len(output_))
    print(output_[0].shape)
    print(output_[1].shape)
    print(output_[2].shape)
    print(output_[3].shape)
    print(output_[4].shape)
    print(output_[5].shape)
    print(output_[6].shape)