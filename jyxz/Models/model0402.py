...
class LPGSequence(nn.Module):
    def __init__(self, feature_channels=32, num_resblocks=3, n=5):
        ...
        self.lpg = LPGModule(in_channels=4, feature_channels=feature_channels, num_resblocks=num_resblocks)
        self.N = n
    ...
    def forward(self, input_image):
        ...
        for i in range(self.N):
            # print(i)
            F_n, feat = self.lpg(input_image, F_prev)
            outputs.append(feat)
            F_prev = F_n
...
class MaskFusion(nn.Module):
    def __init__(self, dim, num_heads=4, bias=False):
        ...
        self.mask_conv = nn.Conv2d(1, dim, kernel_size=3, stride=1, padding=1, bias=bias)
        self.input_conv = nn.Conv2d(3, dim, kernel_size=3, stride=1, padding=1, bias=bias)
        self.output_conv = nn.Conv2d(dim, 3, kernel_size=3, stride=1, padding=1, bias=bias)

    def forward(self, feature, mask, input):
        fea1 = self.mask_conv(mask) * self.input_conv(input) + feature
        fea2 = fea1 + feature
        ouput = self.output_conv(fea2)
        return ouput
        # print(x.shape)
class SpatialAttention(nn.Module):
    def __init__(self, hiddem_dim, kernel_size=7):
        ...
        self.conv2 = nn.Conv2d(32, 1, kernel_size, padding=padding, bias=False)  # 7,3     3,1
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.LPG_block = LPGSequence(feature_channels=32, num_resblocks=3, n=5)
        # Encoder
        ...
        # mask decoder
        self.mask_decoderlayer = BasicUformerLayer(dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=1,
                                                num_heads=num_heads[8],
