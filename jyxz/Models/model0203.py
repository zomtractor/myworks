...
class CNNTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        cnn_x = x.permute(0, 3, 1, 2)

        # x_windows = window_partition(x, self.win_size)  # nW*B, win_size, win_size, C  N*C->C

        attn_result = self.attn(x)  # nW*B, win_size*win_size, C
        cnn_result = self.cnn(cnn_x)
        # print("shape")
        # print(attn_result.shape)
        # print(cnn_result.shape)
        result = attn_result + cnn_result
        # x = window_reverse(attn_windows, self.win_size, H, W)  # B H' W' C
...
class CNN_TransformerLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False):
        ...
        self.use_checkpoint = use_checkpoint
        self.blocks = nn.ModuleList([
            CNNTransformerBlock(dim=dim, input_resolution=input_resolution,
                                    num_heads=num_heads, win_size=win_size,
                                    mlp_ratio=mlp_ratio,
                                    drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                    norm_layer=norm_layer,
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        # mask decoder
        self.mask_decoderlayer = CNN_TransformerLayer(dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=1,
                                                num_heads=num_heads[8],
