...
from Modules.MWSAttention import MWSAttention
...
class NoWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8,
        ...
        self.win_size = win_size
        self.mlp_ratio = mlp_ratio
        self.dim_head = int(self.dim / self.num_heads)
        self.norm1 = norm_layer(dim)
        self.attn = MDTAttention(dim=dim, num_heads=num_heads)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
    ...
    def forward(self, x):
        ...
        # FFN
        x = shortcut + self.alpha * self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
class MWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8,
                 mlp_ratio=4., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        self.dim_head = int(self.dim / self.num_heads)
        self.norm1 = norm_layer(dim)
        self.attn = MWSAttention(dim=dim,
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        # mask decoder
        self.mask_decoderlayer = MWinTransformerBlock(dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=1,
                                                num_heads=num_heads[8],
