...
class StripBlock(nn.Module):
    ...
    def forward(self, x):   
        attn = self.ac1(self.conv0(x))
        attn = self.ac2(self.conv_spatial1(attn))
        attn = self.ac3(self.conv_spatial2(attn))
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4., k1=1, k2=19, drop=0.,drop_path=0., act_layer=nn.GELU):
        ...
        # self.norm1 = nn.GroupNorm(dim//2, dim)
...
class MNet(nn.Module):
    def __init__(self, dim=32,num_blocks=[1,2,8,2]):
        super().__init__()
        self.dwt  = WavePool(3)
