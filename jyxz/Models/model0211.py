...
class MaskFusion(nn.Module):
    ...
    def forward(self, feature, mask, input):
        b, n, c = feature.shape
        # print("model show", feature.shape)
        e = int(math.sqrt(n))
        feature = feature.transpose(-1, -2).view(b, c, e, e)
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.mask_guide = MaskAttentionUformerLayer(dim=embed_dim * 2,
                                                ...
                                                norm_layer=norm_layer,
                                                use_checkpoint=use_checkpoint)
        self.mask_fusion = MaskFusion(embed_dim * 2)
    ...
    def forward(self, x):
        ...
        y_feature, m = self.mask_guide(deconv3, m_feature)
        y = self.mask_fusion(y_feature, mm, x)
