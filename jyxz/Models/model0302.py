...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class hori_ConvBNAct(nn.Module):
    def __init__(self, hidden_dim, k, s=1, p=None, d=1, act=True):
        super().__init__()
        if p is None:
            p=(k//2,0)
        self.conv = nn.Conv2d(hidden_dim, hidden_dim, (k, 1), padding=p, groups=hidden_dim)
        self.act  = nn.GELU() if act else nn.Identity()
    def forward(self, x):
        return self.act(self.conv(x))
class verti_ConvBNAct(nn.Module):
    def __init__(self, hidden_dim, k, s=1, p=None, d=1, act=True):
        super().__init__()
        if p is None:
            p=(0, k//2)
        self.conv = nn.Conv2d(hidden_dim, hidden_dim, (1, k), stride = s, padding=p, groups=hidden_dim)
        self.act  = nn.GELU() if act else nn.Identity()
    def forward(self, x):
        return self.act(self.conv(x))
...
class ChannelContentMixer(nn.Module):
    def __init__(self, dim):
        ...
        self.hori_dwconv = hori_ConvBNAct(hidden_dim, 15)
        self.verti_dwconv = verti_ConvBNAct(hidden_dim, 15)
        self.fc2 =nn.Conv2d(hidden_dim, dim, 1)

    def forward(self, x):
        x = self.fc1(x)
        x = self.dwconv(x)
        x1 = self.hori_dwconv(x)
        x2 = self.verti_dwconv(x)
        x = x + x1 + x2
        x = self.fc2(x)
        return x
