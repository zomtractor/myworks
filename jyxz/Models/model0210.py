...
from Modules.MWSAttention01 import MWSAttention
from Modules.DSConv import DSConv_pro
from Modules.MultiScaleFeatureExtractior import MultiScaleFeatureExtractor
...
class InputProj(nn.Module):
    def __init__(self, in_channel=3, out_channel=64, kernel_size=3, stride=1, norm_layer=None, act_layer=nn.LeakyReLU):
        ...
        self.proj_1 = nn.Sequential(
            nn.Conv2d(out_channel * 5, out_channel, kernel_size=3, stride=stride, padding=kernel_size // 2),
            act_layer(inplace=True)
        )
        ...
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.streak_proj = DSConv_pro(out_channel,out_channel)
        self.scatter_proj = MultiScaleFeatureExtractor(out_channel,out_channel,[3, 5, 7])
        

    def forward(self, x):
        B, C, H, W = x.shape
        x = self.proj(x)
        streat_feat = self.streak_proj(x)
        sc_feat = self.scatter_proj(x)
        x = torch.cat((x,streat_feat,sc_feat), dim=1)
        x = self.proj_1(x)
        x = x.flatten(2).transpose(1, 2).contiguous()  # B H*W C
        if self.norm is not None:
            x = self.norm(x)
        return x, streat_feat, sc_feat
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=32, depths=depths,
                                win_size=8, token_projection='linear').cuda(0)
    ...
    input_ = torch.randn(1, 3, input_size, input_size).cuda(0)
