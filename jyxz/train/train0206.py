...
from Models.model0206 import Uformer
#改model和config
...
# load yaml
with open(file="../configs/config0206.yaml", mode="r") as config:
...
# load model
model_base = Uformer(img_size=256, embed_dim=32, win_size=16, token_mlp='leff',
                     depths=[1, 2, 8, 8, 4, 8, 8, 2, 1], dd_in=3, batch_size=OPT['BATCH'])  #batch01
...
if TRAIN['RESUME']:
    before_path = TRAIN['BATCH_01_PATH']
    path_chk_rest = utils.get_last_path(before_path, '_latest.pth')
