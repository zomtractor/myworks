...
from Models.model0403 import Uformer
#改model和config
...
# load yaml
with open(file="../configs/config0403.yaml", mode="r") as config:
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        flare = 0.299 * R + 0.587 * G + 0.114 * B
        # print("flare.shape",flare.shape)
        mask_pred = model_base(input_)
        name = data[3]


        # 正常计算loss
        char_l1_loss = CharLoss(mask_pred, flare)
        ssim_loss = (1 - SSIMLoss(mask_pred, flare))
        # vgg_loss = VGGLoss(mask_pred, flare)
        image_loss = char_l1_loss + ssim_loss
    ...
    print("------------------------------------------------------------------")
    print(
        "Epoch: {}\tTime: {:.4f}\tImage Loss: {:.4f}\ttLossTotal: {:.4f}\tLearningRate {:.8f}"
