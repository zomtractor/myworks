...
from Models.model0402 import Uformer
#改model和config
...
# load yaml
with open(file="../configs/config0402.yaml", mode="r") as config:
...
model_base = Uformer(img_size=256, embed_dim=32, win_size=8, token_mlp='leff',
                     depths=[1, 1, 1, 1, 8, 1, 1, 1, 1], dd_in=3, batch_size=OPT['BATCH'])
...
if TRAIN['RESUME']:
    path_chk_rest = utils.get_last_path(model_path, '_bestPSNR.pth')
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        restored, mask_pred= model_base(input_)
        name = data[3]
    ...
    if epoch % TRAIN['VAL_AFTER_EVERY'] == 0:
        ...
        for ii, data_val in enumerate(val_loader, 0):
            ...
            with torch.no_grad():
                restored, mask = model_base(input_)
                # if h_pad != 0:
