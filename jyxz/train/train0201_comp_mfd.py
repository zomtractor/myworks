...
sys.path.append("..")
from Models.MFDModels.model import Model
import utils
from utils import *
from lightning.fabric import Fabric
# from Models.MFDModel.model import *
...
# load yaml
with open(file="../configs/config0201_comp_mfd.yaml", mode="r") as config:
...
if TRAIN['RESUME']:
    path_chk_rest = utils.get_last_path(resume_path, '_latest.pth')
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        loss_psnr = criterion_psnr(restored, gt)
        loss_ssim = 1 - criterion_ssim(restored, gt, data_range=1)
        loss_lpips = criterion_lpips(restored.mul(2).sub(
            1).clamp(-1, 1), gt.mul(2).sub(1).clamp(-1, 1))
        ...
        epoch_total_loss += train_loss.item()
    ...
    print(
        ...
        .format(epoch, time.time() - epoch_start_time, train_loss, epoch_total_loss, scheduler.get_lr()[0]))
    print("------------------------------------------------------------------")
