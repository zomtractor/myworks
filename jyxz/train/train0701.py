...
from Models.model0701 import Uformer
...
# load yaml
with open(file="../configs/config0701.yaml", mode="r") as config:
...
# load model
model_base = Uformer(img_size=256, embed_dim=16, win_size=8, token_mlp='leff',
                     depths=[1, 1, 1, 4, 2, 4, 1, 1, 1], dd_in=3, batch_size=OPT['BATCH']) #model13
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        restored, mask_pred, mask_tensor = model_base(input_)
        name = data[3]
        ...
        mask_loss = structure_loss(mask_pred, mask)
        # mask_loss = 0.01 /  epoch * mask_loss  
        mask_loss = 0.2 / (5 * epoch) * mask_loss  # 13
        # mask_loss = 0.1 / (5 * epoch) * mask_loss  # 14
        # mask_loss = 0.5 / (2 * epoch) * mask_loss   # 0201_06
        total = image_loss +  mask_loss
    ...
    if epoch % TRAIN['VAL_AFTER_EVERY'] == 0:
        ...
        for ii, data_val in enumerate(val_loader, 0):
            ...
            with torch.no_grad():
                restored, mask, mask_tensor = model_base(input_)
                if h_pad != 0:
            ...
            heatmap_path = os.path.join(TRAIN['TRAIN_HEATMAP'], model_name)
            draw_feature_map(mask_tensor, heatmap_path, data_val[2][0] + '.png', model_name)
            mask_path = os.path.join(TRAIN['TRAIN_MASK'], model_name)
            if not os.path.exists(mask_path):
                os.makedirs(mask_path)
            cv2.imwrite(os.path.join(mask_path, data_val[2][0] + '.png'), mask * 255)
    ...
    print("------------------------------------------------------------------")
    print(
        "Epoch: {}\tTime: {:.4f}\tImage Loss: {:.4f}\tMask Loss: {:.4f}\tLossTotal: {:.4f}\tLearningRate {:.8f}"
        .format(epoch, time.time() - epoch_start_time, image_loss, mask_loss, epoch_total_loss, scheduler.get_lr()[0]))
    print("------------------------------------------------------------------")
