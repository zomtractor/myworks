...
def validate(config, name, model_restored, val_loader, record_dict, loss_fn):
    ...
    for ii, data_val in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            restored = model_restored(input_padded)[0][2]
...
if __name__ == '__main__':
    ...
    combined_gt_loss2 = CombinedLoss(Train['LOSS']).cuda()
    combined_gt_loss3 = CombinedLoss(Train['LOSS']).cuda()
    combined_flare_loss1 = CombinedLoss(Train['LOSS']).cuda()
    combined_flare_loss2 = CombinedLoss(Train['LOSS']).cuda()
    ...
    for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            input_ = data[1].cuda()
            flare =  data[2].cuda()
            restored,flares = model_restored(input_)

            loss3_gt = combined_gt_loss3(restored[0], F.interpolate(target, scale_factor=0.25, mode='bilinear', align_corners=False))
            loss2_gt = combined_gt_loss2(restored[1], F.interpolate(target, scale_factor=0.5, mode='bilinear', align_corners=False))
            loss1_gt = combined_gt_loss1(restored[2], target)
            loss3_flare = combined_flare_loss3(flares[0], F.interpolate(flare, scale_factor=0.25, mode='bilinear', align_corners=False))
            loss2_flare = combined_flare_loss2(flares[1], F.interpolate(flare, scale_factor=0.5, mode='bilinear', align_corners=False))
            loss1_flare = combined_flare_loss1(flares[2], flare)
            loss = loss1_gt+0.5*loss2_gt+0.2*loss3_gt+0.5*loss1_flare+0.2*loss2_flare+0.1*loss3_flare
            # Back propagation
            # loss.backward()
        ...
        if fabric.is_global_zero:
            ...
            combined_gt_loss1.merge(combined_gt_loss2)
            combined_gt_loss1.merge(combined_gt_loss3)
            combined_gt_loss1.print_cumulative_loss()
            combined_gt_loss1.clear_cumulative_loss()
            combined_gt_loss2.clear_cumulative_loss()
            combined_gt_loss3.clear_cumulative_loss()
