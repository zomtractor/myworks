...
class LeFF(nn.Module):
    def __init__(self, dim, hidden_dim, act_layer=nn.GELU, drop=0.):
        ...
        self.pw = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, stride=1, padding=0, dilation=1, groups=1,
                            bias=False)
        self.dim = dim
    ...
    def forward(self, x):
        ...
        x = self.linear1(x)  # B N C --> B N C

        x = rearrange(x, ' b (h w) (c) -> b c h w ', h=hh, w=hh)
        x2 = self.dwconv(x)  # B C H W --> B C H W
        x2 = rearrange(x2, ' b c h w -> b (h w) c', h=hh, w=hh)
        x = self.linear2(x2)  # B N C --> B N C
        return x
...
#######################################################
########### Frequency-Enhanced Unit (FEU) #############
class FourierUnit(nn.Module):
    def __init__(self, hidden_dim, fft_norm='ortho'):
        super(FourierUnit, self).__init__()
        self.conv_layer = torch.nn.Conv2d(hidden_dim * 2, hidden_dim * 2, 1, 1, 0)
...
class FEU(nn.Module):
    def __init__(self, hidden_dim, last_conv=False):
        super(FEU, self).__init__()
        self.last_conv = last_conv

        self.conv1 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim // 2, 1, 1, 0),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
        self.fu = FourierUnit(hidden_dim // 2)

        self.conv2 = torch.nn.Conv2d(hidden_dim // 2, hidden_dim, 1, 1, 0)

        if self.last_conv:
            self.last_conv = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1)
...
#############################################################
###########Spatial-Frequency Enhanced Module (SFEM)##########
class SFEM(nn.Module):
    def __init__(self, hidden_dim, red=1):
        super(SFEM, self).__init__()
        self.S = SRU(hidden_dim)
        self.F = FEU(hidden_dim)
...
#########################################
########### FGRT #############
class New_TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        out = rearrange(a2, ' b c h w -> b (h w) c')
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        self.blocks = nn.ModuleList([
            New_TransformerBlock(dim=dim, input_resolution=input_resolution,
...
class FGRNet(nn.Module):
    def __init__(self, img_size=128, img_ch=3, output_ch=3,
                 embed_dim=32, depths=[1, 2, 2, 2, 2, 2, 2, 2, 1], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
