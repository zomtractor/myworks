...
class UBlock(nn.Module):
    def __init__(self, in_channels=3, base_channels=32,in_height=512,in_width=512,weight_connect=True):
        ...
        self.dbp4 = DBlockFlare(base_channels * 8)
        self.proj_laplacian4 = nn.Conv2d(3, base_channels * 8, kernel_size=3, padding=1)
        self.up4 = UpSample(base_channels * 16)
        self.db4 = FeatureBlock(base_channels * 8)
        self.dbp3 = DBlockFlare(base_channels * 4)
        self.proj_laplacian3 = nn.Conv2d(3, base_channels * 4, kernel_size=3, padding=1)
        self.up3 = UpSample(base_channels * 8)
        self.db3 = FeatureBlock(base_channels * 4)
        self.dbp2 = DBlockFlare(base_channels * 2)
        self.proj_laplacian2 = nn.Conv2d(3, base_channels * 2, kernel_size=3, padding=1)
        self.up2 = UpSample(base_channels * 4)
        self.db2 = FeatureBlock(base_channels * 2)
        self.dbp1 = DBlockFlare(base_channels * 1)
        self.proj_laplacian1 = nn.Conv2d(3, base_channels * 1, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        out = self.up4(out)
        out = self.dbp4(out+self.proj_laplacian4(laplacian[3]))
        out = self.db4(out)
        out = self.up3(out)
        out = self.dbp3(out+self.proj_laplacian3(laplacian[2]))
        out = self.db3(out)
        out = self.up2(out)
        out = self.dbp2(out+self.proj_laplacian2(laplacian[1]))
        out = self.db2(out)
        out = self.up1(out)
        out = self.dbp1(out+self.proj_laplacian1(laplacian[0]))
        out = self.db1(out)
        out = self.tail(out)
