...
class MyNet2_2_pmul(nn.Module):
    def __init__(self, base_channels=16, num_block=3, num_bottleneck=2, encoders=[1,2,3],decoders=[3,1,1]):
        super(MyNet2_2_pmul, self).__init__()
        ...
        self.ebs = nn.ModuleList(
            [nn.Sequential(
                *[EBlock(base_channels * 2 ** i) for _ in range(encoders[i])])
        ...
        self.dbs_pred =nn.ModuleList(
            [nn.Sequential(
                *[DBlock(base_channels * 2 ** (i)) for _ in range(decoders[i])])
            for i in range(num_block)])
