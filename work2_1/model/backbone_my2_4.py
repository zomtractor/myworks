...
class MyNet2_4(nn.Module):
    def __init__(self, base_channels=16, num_block=3, num_bottleneck=2):
        super(MyNet2_4, self).__init__()
        self.num_block = num_block
        self.num_bottleneck = num_bottleneck
        self.proj_in = BasicConv(3, base_channels, kernel_size=3, padding=1)
        self.proj_laplacian = nn.ModuleList([BasicConv(3, base_channels * (i+1), kernel_size=3, padding=1) for i in
                               range(num_block)])
        self.ebs = nn.ModuleList([EBlock(base_channels * (i+1)) for i in range(num_block)])
        self.ebs_flare = nn.ModuleList([EBlockFlare(base_channels * (i+1)) for i in range(num_block)])
        self.bottleneck = nn.ModuleList([BottleNeck(base_channels * (num_block+1)) for _ in range(num_bottleneck)])
        self.dbs_pred = nn.ModuleList([DBlock(base_channels *(i+1)*2) for i in range(num_block)])
        self.out_reduce = nn.ModuleList([BasicConv(base_channels* (i+1)*2,base_channels * (i+1)) for i in range(num_block)])

        self.ups = nn.ModuleList([UpSample(base_channels * (i+2), base_channels * (i+1)) for i in range(num_block)])
        self.downs = nn.ModuleList([DownSample(base_channels * (i+1), base_channels * (i+2)) for i in range(num_block)])
