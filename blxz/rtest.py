...
test_dir='/mnt/zbl/deflare/DataSet/test/my2'
model_restored.cuda()
utils.mkdir("/mnt/zbl/deflare/code/All_result/ours/rtest_result")
utils.mkdir("/mnt/zbl/deflare/code/All_result/ours/rtest_mask_result")
## DataLoaders
...
for ii, data_test in enumerate(test_loader, 0):
    ...
    for batch in range(len(restored)):
        ...
        cv2.imwrite(os.path.join('/mnt/zbl/deflare/code/All_result/ours/rtest_result', data_test[1][batch] + '.png'), cv2.cvtColor(restored_img, cv2.COLOR_RGB2BGR))
    mask = mask.sigmoid().data.cpu().numpy().squeeze()
    mask = (mask - mask.min()) / (mask.max() - mask.min() + 1e-8)
    cv2.imwrite(os.path.join('/mnt/zbl/deflare/code/All_result/ours/rtest_mask_result', data_test[1][0] + '.png'), mask * 255)
