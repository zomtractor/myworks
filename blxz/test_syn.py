...
def getResult():
    ...
    for ii, data_test in enumerate(test_loader, 0):
        ...
        for batch in range(len(restored)):
            ...
            cv2.imwrite(os.path.join('./test_result_syn', data_test[2][batch] + '.png'), cv2.cvtColor(restored_img, cv2.COLOR_RGB2BGR))
        mask = mask.sigmoid().data.cpu().numpy().squeeze()
        mask = (mask - mask.min()) / (mask.max() - mask.min() + 1e-8)
        cv2.imwrite(os.path.join('./mask_result_syn', data_test[2][0] + '.png'), mask * 255)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt',type=str,default='/mnt/sdb/zbl/FlareDataSet2/test/synthetic/gt')
